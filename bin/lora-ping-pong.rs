        ...
        if waiting_for_reply {
            ...
            match result {
                Ok(Ok((envelope, status))) => {
                    ...
                    if envelope.msg_type == MessageType::Reply && envelope.seq == expected_seq {
                        let mut metrics_ref = metrics.lock().await;
                        metrics_ref.last_rssi = rssi;
                        metrics_ref.avg_rssi = if rssi_samples > 0 { total_rssi / rssi_samples as i32 } else { 0 };
                        drop(metrics_ref);

                        info!("Reply received: seq={}, rssi={}", envelope.seq, rssi);
                        waiting_for_reply = false;
                        seq = seq.wrapping_add(1);
                        expected_seq = seq;
                    } else {
                        info!("Ignored: seq={}, type={:?}", envelope.seq, envelope.msg_type);
                    }
                }
                Ok(Err(e)) => {
                    error!("Receive error: {:?}", e);
                ...
                Err(_) => {
                    let mut metrics_ref = metrics.lock().await;
                    metrics_ref.packets_lost = metrics_ref.packets_lost.saturating_add(1);
                    drop(metrics_ref);

                    info!("Timeout waiting for reply, packet lost");
                    waiting_for_reply = false;
                    seq = seq.wrapping_add(1);
                    expected_seq = seq;
                }
...
async fn main(_spawner: Spawner) {
    ...
    if let Err(e) = display.show_message("LoRa Range\nTest Ready") {
        error!("Failed to show initial message: {:?}", e);
    }

    let _ = _spawner.spawn(task_send(channel, lora, metrics));
    let _ = _spawner.spawn(task_receive(lora, metrics));
    let _ = _spawner.spawn(task_ping_pong(channel, lora, rng, metrics));
    ...
    loop {
        if prg_button.is_low() && !button_was_pressed {
            ...
            info!("PRG pressed - reset metrics");

            let mut metrics_ref = metrics.lock().await;
            *metrics_ref = LoraMetrics::default();
            drop(metrics_ref);
